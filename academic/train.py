...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=cfg.env.BATCH_SIZE, num_workers=8, sampler=None
    )
    valid_loader = torch.utils.data.DataLoader(
        valid_dataset, batch_size=cfg.env.BATCH_SIZE, num_workers=8, sampler=None
    )
            ...
            if global_step % cfg.env.save_results_steps == 0:
                ...
                with torch.no_grad():
                    ...
                    save_audio(
                        out,
                        os.path.join(
                            cfg.env.PATH, "out_{:010d}.wav".format(global_step)
                        ),
                        cfg.env.sr,
                        rescale=rescale,
                    )
